...
mongodb.connectDB(async err => {
    ...
    router.get('/getAllAssignJob', function(req, res, next) {
        db.collection('AssignJob').find({}).toArray(function(err, data) {
            if (err) {
                throw err;
            }
    ...
    router.get('/getAllWorkorder', function(req, res, next) {
        ...
        db.collection('Workorder').find({ JobDefined: false }).toArray(function(err, data) {
            if (err) {
                throw err;
            }
            data.forEach(element => {
    ...
    router.get('/getWorkOrder/:WorkOrderNo', function(req, res, next) {
        ...
        db.collection('AssignJob').find({ WorkOrderNo: WorkOrderNo }).toArray(function(err, data) {
            if (data.length > 0) {
                res.status(403).json({ message: "Selected Work Order is Assigned already" });
                res.end();
            } else {
                db.collection('Workorder').find({ WorkOrderNo: WorkOrderNo }).toArray(function(err, data) {
                    if (err) {
                        throw err;
                    }
                    data.forEach(element => {
    ...
    router.post('/add', async(req, res, next) => {
        ...
        db.collection('AssignJob').insertOne(req.body, {}, function(err, doc) {
            if (err) {
                throw err;
            }
            ...
            if (xResult) {
                ...
                db.collection('Workorder').updateOne(myquery, newvalues, {}, function(err, doc) {
                    ...
                    db.collection('VehicleAssignmentHistory').insertMany(xAssignList, {}, function(err, doc) {
                        if (err) {
                            throw err;
                        }
    ...
    router.post('/update/:id', function(req, res, next) {
        ...
        db.collection('AssignJob').updateOne({ _id: objectId }, { $set: req.body }, {}, function(err, doc) {
            if (err) {
                throw err;
            }
            if (xResult) {
    ...
    router.post('/delete/:id', function(req, res, next) {
        objectId = new ObjectId(req.params.id);
        db.collection('AssignJob').deleteOne({ _id: objectId }, function(err, doc) {
            if (err) {
                throw err;
            }
            var myquery = { "WorkOrderNo": req.body.WorkOrderNo };
