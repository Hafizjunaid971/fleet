...
mongodb.connectDB(async err => {
    ...
    router.get('/getUniqueNumber/:sequenceName', function(req, res, next) {
        ...
        db.collection('CranCustomercounters').find({ _id: sequenceName }).toArray((err, doc) => {
            if (doc && doc.length) {
                sequence_value = doc[0].sequence_value + 1;
                db.collection('CranCustomercounters').updateOne({ _id: sequenceName }, { $set: { sequence_value: sequence_value } }, {}, function(err, counter) {
                    if (err) {
                        console.log(err);
                    }
                ...
                db.collection('CranCustomercounters').insertOne(obj, {}, function(err, counter) {
                    if (err) {
                        console.log(err);
                    }
                    const response = JSON.stringify({ data: sequence_value, status: 200 });
    ...
    router.get('/getAllCranCustomer', function(req, res, next) {
        db.collection('CranCustomerSetup').find({}).toArray(function(err, data) {
            if (err) {
                throw err;
            }
    ...
    router.post('/add', function(req, res, next) {
        ...
        db.collection('CranCustomerSetup').find({ Name: Name }).toArray(function(err, data) {
            if (err) {
                throw err;
            }
            ...
            if (data.length > 0) {
                ...
                db.collection('CranCustomerSetup').insertOne(req.body, {}, function(err, doc) {
                    if (err) {
                        throw err;
                    }
    ...
    router.post('/update/:id', function(req, res, next) {
        ...
        delete req.body._id;
        db.collection('CranCustomerSetup').updateOne({ _id: objectId }, { $set: req.body }, {}, function(err, doc) {
            if (err) {
                throw err;
            }
            const response = JSON.stringify({ data: req.body, status: 200 });
    ...
    router.post('/delete/:id', function(req, res, next) {
        ...
        db.collection('CranCustomerSetup').deleteOne({ _id: objectId }, function(err, doc) {
            if (err) {
                throw err;
            }
            const response = JSON.stringify({ data: req.body, status: 200 });
