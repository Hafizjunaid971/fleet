...
mongodb.connectDB(async err => {
    ...
    router.get('/getAllTransporter', function(req, res, next) {
        db.collection('transportersetup').find({}).toArray(function(err, data) {
            if (err) {
                throw err;
            }
    ...
    router.post('/add', function(req, res, next) {
        ...
        db.collection('transportersetup').find({ Name: Name }).toArray(function(err, data) {
            if (err) {
                throw err;
            }

            if (data.length > 0) {
                res.status(403).json({ message: "Transporter already registered" });
                res.end();
            } else {
                req.body.CreatedDate = dateFormat(new Date(), 'isoDateTime');
                db.collection('transportersetup').insertOne(req.body, {}, function(err, doc) {
                    if (err) {
                        throw err;
                    }
    ...
    router.post('/update/:id', function(req, res, next) {
        ...
        delete req.body._id;
        db.collection('transportersetup').updateOne({ _id: objectId }, { $set: req.body }, {}, function(err, doc) {
            if (err) {
                throw err;
            }
            const response = JSON.stringify({ data: req.body, status: 200 });
    ...
    router.post('/delete/:id', function(req, res, next) {
        objectId = new ObjectId(req.params.id);
        db.collection('transportersetup').deleteOne({ _id: objectId }, function(err, doc) {
            if (err) {
                throw err;
            }
            const response = JSON.stringify({ data: req.body, status: 200 });
