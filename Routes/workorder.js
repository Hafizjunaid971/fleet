...
mongodb.connectDB(async err => {
    ...
    router.get('/ValidateWO/:WorkOrderNo/:Type', function(req, res, next) {
        ...
        if (Type === "WO") {
            db.collection('SalesInvoice').find({ "WorkOrderList.WorkOrderNo": WorkOrderNo }).toArray(function(err, data) {
                if (data.length > 0) {
    ...
    router.post('/add', async(req, res, next) => {
        ...
        req.body.Advance=parseFloat(req.body.Advance);
        db.collection('Workorder').insertOne(req.body, {}, function(eSrr, doc) {
            if (err) {
                throw err;
            }
            var list = [];
            var obj = {
                InvoiceNumber: req.body.WorkOrderNo,
            ...
            if(req.body.Advance > 0){
                var obj = {
                    InvoiceNumber: req.body.WorkOrderNo,
                    Date: req.body.CreatedDate,
                    Customer: req.body.Customer.Name,
                    Amount: req.body.Receivable - req.body.Payable,
                    Type: "Workorder",
                    Flag: 'Quantity In',
                }
                list.push(obj);
            ...
            if (req.body.Paid > 0) {
                var obj = {
                    ...
                    Date: req.body.CreatedDate,
                    Customer: req.body.Customer.Name,
                    Particulars: "Customer Payment against WO: " + req.body.WorkOrderNo,
    ...
    router.post('/update/:id', function(req, res, next) {
        ...
        db.collection('Workorder').updateOne({ _id: objectId }, { $set: req.body }, {}, function(err, doc) {
            if (err) {
                throw err;
            }
    ...
    router.post('/delete/:id', function(req, res, next) {
        objectId = new ObjectId(req.params.id);
        db.collection('Workorder').deleteOne({ _id: objectId }, function(err, doc) {
            if (err) {
                throw err;
            }
            const repsone = JSON.stringify({ data: req.body, status: 200 });
